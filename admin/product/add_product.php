...
if ($_SERVER["REQUEST_METHOD"]=="POST") 
{
  ...
  if (file_exists($target_file))
  {
      echo "<script>alert('Files đã tồn tại trên server');location.href='add_product.php'";
		...
		$mysql = "INSERT INTO `product` ( `name`, `price`, `author`, `quanlity`, `category_id`, `img`) VALUES ( '$name', '$price', '$author', '$quanlity', '$category_id', '$name_file');";
        $add = $conn->query($mysql);
		if ($add>0) 
		{
			move_uploaded_file($_FILES["fileupload"]["tmp_name"], $target_file);
			echo "<script>alert('Thêm mới thành công');location.href='quanlisp.php'</script>";
		}
		else
		{
...
<body>
	...
	<h2>Input add product's information :</h2>
	...
	<select name="category_id" id="">
		...
		'category_id'] ?>"><?php echo $item['category_name']; ?></option>
	<?php endforeach; ?>
	</select>
	<h3>img:</h3>
